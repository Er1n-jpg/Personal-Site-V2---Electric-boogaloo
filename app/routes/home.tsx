...
function App() {
  ...
  const handleToaster = async (e: { preventDefault: () => void; target: any }) => {
    ...
    try {
      ...
      if (response.ok) {
        setToast({ type: 'success', message: 'Message sent! Thanks for the yap 💌' })
        form.reset()
      } else {
        setToast({ type: 'error', message: 'Oops, something went wrong. Try again?'  })
      }
    } catch (err) {
      ...
      <div className="bg-[#F8ECDC] flex flex-col top-0 w-full items-center justify-center">
        ...
        <div className="flex items-center justify-center">
          <div className="w-[80rem] bg-[#304076] flex flex-col items-start py-10 px-10 text-[#F8ECDC] rounded-xl">
            <div className="flex flex-row justify-between items-center gap-[30rem]">
            ...
            <div className="flex flex-row mt-10 mb-5">
              <form action="https://formspree.io/f/mkjnvyvp" method="POST" onSubmit={handleToaster} className="flex flex-row mb-5">
                <input type="text" name="message" className="w-[60rem] h-20 bg-[#F8ECDC] rounded-lg mr-5 text-[#304076] text-4xl font-newsreader justify-center py-5 px-5"
              ...
              {toastState && (
                <div
                  className={`fixed bottom-8 right-8 z-50 px-6 py-4 rounded-lg shadow-lg text-lg font-newsreader transition-opacity duration-300
            ${toastState.type === 'success' ? 'bg-[#304076] text-[#F8ECDC]' : 'bg-red-500 font-newsreader text-white'}`}
                >
                  {toastState.message}
                </div>
              )}
